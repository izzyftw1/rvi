-- Fix auto_generate_work_order_from_line_item to remove non-existent priority column
DROP TRIGGER IF EXISTS trigger_auto_generate_wo_from_line_item ON sales_order_line_items;
DROP FUNCTION IF EXISTS auto_generate_work_order_from_line_item();
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND OLD.status != 'approved' AND NEW.status = 'approved' AND NEW.work_order_id IS NULL) THEN
    ...
    -- Insert new work order with line item-specific details (removed priority column)
