...
import { Calendar, Search, ArrowUpDown, Package } from "lucide-react";
...
interface ExternalProcessingDetailDrawerProps {
...
export const ExternalProcessingDetailDrawer = ({
  processType,
  processLabel,
  open,
  onClose,
}: ExternalProcessingDetailDrawerProps) => {
  ...
  useEffect(() => {
    if (open && processType) {
      ...
      const channel = supabase
        .channel('external-processing-detail')
        .on(
          'postgres_changes',
          {
            event: '*',
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant={variants[status as keyof typeof variants] || 'outline'} className={cn("font-medium", colors[status as keyof typeof colors])}>
        {status}
      </Badge>
    );
  };
  ...
  return (
    <Drawer open={open} onOpenChange={onClose}>
      <DrawerContent className="h-[90vh]">
        <DrawerHeader className="border-b bg-muted/50">
          <DrawerTitle className="text-2xl font-semibold">
            {processLabel} Jobs in Progress
          </DrawerTitle>
        ...
        <div className="p-6 overflow-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-lg">No jobs found for {processLabel}</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  >
                    ...
                    <TableCell>
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          navigate(`/work-orders/${job.wo_id}`);
                          onClose();
                        }}
                      >
                        View WO
