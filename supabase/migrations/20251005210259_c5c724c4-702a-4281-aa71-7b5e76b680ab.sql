...
-- Create user_audit_log table for tracking all user actions
...
BEGIN
  ...
  INSERT INTO public.user_audit_log (user_id, action_type, module, action_details)
  VALUES (NEW.id, 'login', 'auth', jsonb_build_object('email', NEW.email));
  
  RETURN NEW;
END;
$$;

-- Trigger to log user logins (on auth.users table)
-- Note: This trigger is on auth schema which we typically can't modify directly
-- Instead, we'll log logins from the application layer
