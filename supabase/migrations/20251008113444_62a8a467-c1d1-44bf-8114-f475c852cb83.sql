-- Fix the auto_generate_work_order_from_line_item trigger by dropping dependencies first
-- Drop all triggers that depend on the function
DROP TRIGGER IF EXISTS auto_generate_work_order_trigger ON sales_order_line_items;
DROP TRIGGER IF EXISTS trigger_auto_generate_work_order ON sales_order_line_items;
DROP TRIGGER IF EXISTS trigger_auto_generate_wo_from_line_item ON sales_order_line_items;

-- Now we can drop the function
DROP FUNCTION IF EXISTS auto_generate_work_order_from_line_item() CASCADE;
